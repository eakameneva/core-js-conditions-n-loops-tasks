...
function isPalindrome(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    convertedString = str[i] + convertedString;
  }
...
function isContainNumber(num, digit) {
  ...
  for (let i = 0; i < stringedNumber.length; i += 1) {
    if (stringedNumber[i] === String(digit)) {
      return true;
    }
  }
  return false;
...
function getNearestBigger(num) {
  ...
  let end = n - 1;
  while (start < end) {
    const reverseTemp = digits[start];
    digits[start] = digits[end];
    digits[end] = reverseTemp;
    start += 1;
    end -= 1;
  }
